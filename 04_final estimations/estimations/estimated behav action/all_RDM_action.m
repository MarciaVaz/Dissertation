...
load ('estimate_RDM_behav_action.mat');  
comparisons = estimate_RDM_a;
...
figure('units','normalized','WindowState','maximized');   image(rdm_action_mix,'CDataMapping','scaled');C = colorbar;caxis([0 max_val]);       
set(gca,'XTick',1:1:46); set(gca,'xticklabel',table2array(stimuli_names_92(47:92,1)),'fontsize', 6);xtickangle(90);
set(gca,'YTick',1:1:46); set(gca,'yticklabel',table2array(stimuli_names_92(1:46,1)),'fontsize', 6);ytickangle(0);
set(gca, 'TickLength',[0 0]);
axis equal; axis image; 
title(['Action RDM: behaviour data (animals vs tools)'],'fontsize',11);
saveas(gcf,'behav_RDM_action_mixed.png');
save('behav_RDM_action_mixed','rdm_action_mix');
...
rdm_vector_mixed = reshape(rdm_action_mix',1,[]);
...
save RDMs_behav_vector_action_mixed rdm_vector_mixed

%% completed
save RDMs_behav_vector_action_animals_completed rdm_vector_animals_completed
save RDMs_behav_vector_action_tools_completed rdm_vector_tools_completed
