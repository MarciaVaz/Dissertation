...
%
path_action = dir('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\03_fMRI experiment\06_create RDMs\03_partial RDMs\partial action');
file_names_action = path_action(~[path_action.isdir]);
file_names_action = {file_names_action.name};
file_names_action = file_names_action(1:15);

path_semantic = dir('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\03_fMRI experiment\06_create RDMs\03_partial RDMs\partial semantic');
file_names_semantic = path_semantic(~[path_semantic.isdir]);
file_names_semantic = {file_names_semantic.name};
file_names_semantic = file_names_semantic(1:15);

 for each_ROI=1:numel(file_names_action)
    partials_ROI_action = load(file_names_action{each_ROI});
    partials_ROI_action = partials_ROI_action.partials_RDM_A;
    [estimate_RDM_A,evidenceWeight_ltv]=estimateRDMFromStackOfPartials(partials_ROI_action,true);
    
    if each_ROI < 10
%     	save([num2str(0) num2str(each_ROI) '_ROI_estimate_RDM_Action.mat'],'estimate_RDM_A')
        save(fullfile('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\04_final estimations\estimations\estimated fMRI action', [num2str(0) num2str(each_ROI) '_ROI_estimate_RDM_Action.mat']), 'estimate_RDM_A'); %maybe also add extension to backmean
    else
%         save([num2str(each_ROI) '_ROI_estimate_RDM_Action.mat'],'estimate_RDM_A')
        save(fullfile('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\04_final estimations\estimations\estimated fMRI action', [num2str(each_ROI) '_ROI_estimate_RDM_Action.mat']), 'estimate_RDM_A'); %maybe also add extension to backmean
    end
    
    partials_ROI_semantic = load(file_names_semantic{each_ROI});
    partials_ROI_semantic = partials_ROI_semantic.partials_RDM_S;
    [estimate_RDM_S,evidenceWeight_ltv]=estimateRDMFromStackOfPartials(partials_ROI_semantic,true); 

    if each_ROI < 10
%     	save([num2str(0) num2str(each_ROI) '_ROI_estimate_RDM_Semantic.mat'],'estimate_RDM_S')
        save(fullfile('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\04_final estimations\estimations\estimated fMRI semantic', [num2str(0) num2str(each_ROI) '_ROI_estimate_RDM_Semantic.mat']), 'estimate_RDM_S'); %maybe also add extension to backmean
    else
%         save([num2str(each_ROI) '_ROI_estimate_RDM_Semantic.mat'],'estimate_RDM_S')
        save(fullfile('C:\Users\Márcia Vaz\Documents\GitHub\Dissertation\04_final estimations\estimations\estimated fMRI semantic', [num2str(each_ROI) '_ROI_estimate_RDM_Semantic.mat']), 'estimate_RDM_S'); %maybe also add extension to backmean
    end
...
nDissimPartialMats=size(stackOfPartialRDMs_ltv,3); % how many participants
