...
path = dir('C:\Users\Marcia\Desktop\extra dissertation\iMDS_MATLAB\partials euclidean\semantic correct order');
file_names = path(~[path.isdir]);
file_names = {file_names.name};
file_names = file_names(1:15)
% % ,'FileExtensions','.mat'
% dat_mat_files = fileDatastore(path,'ReadFcn',@load)
% 
...
% y = string(file_names)
 ...
 for each_ROI=1:numel(file_names)
    partials_ROI = load(file_names{each_ROI});
    partials_ROI = partials_ROI.partials_RDM_S;
    [estimate_RDM_S,evidenceWeight_ltv]=estimateRDMFromStackOfPartials(partials_ROI,true);
    
    if each_ROI == 1
        save 01_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 2
        save 02_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 3
    	save 03_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 4
    	save 04_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 5
    	save 05_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 6
    	save 06_ROI_estimate_RDM_S_correct_order estimate_RDM_S  
    elseif each_ROI == 7
    	save 07_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 8
    	save 08_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 9
    	save 09_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 10
    	save 10_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 11
    	save 11_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 12
    	save 12_ROI_estimate_RDM_S_correct_order estimate_RDM_S 
    elseif each_ROI == 13
     	save 13_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    elseif each_ROI == 14
    	save 14_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    else    
        save 15_ROI_estimate_RDM_S_correct_order estimate_RDM_S
    end
 end
